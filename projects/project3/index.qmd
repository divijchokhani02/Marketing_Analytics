...
df["pct_fulltime"] = 100 * df["empft"] / df["fte"]
df["pct_fulltime2"] = 100 * df["empft2"] / df["fte2"]
...
The **t statistic** column tests whether the NJ and PA means are statistically different from each other. It is computed as:

$$
t = \frac{\bar{X}_{NJ} - \bar{X}_{PA}}{\sqrt{SE_{NJ}^2 + SE_{PA}^2}}
$$

Intuitively, it measures how large the gap between states is relative to the uncertainty in the estimates. A t statistic close to 0 means the two states look similar on that variable — there is no meaningful difference. A large absolute value (roughly above 2) means the difference is unlikely to be due to chance. For example, the t statistic for starting wage in Wave 2 is 10.8, confirming that NJ wages moved sharply higher relative to PA after the policy change. In contrast, most Wave 1 variables have t statistics close to 0, which is reassuring — it means NJ and PA looked similar *before* the policy, making Pennsylvania a credible comparison group.
...
The replicated NJ coefficient (2.36) is very close to the paper's published result of 2.33 in Table 4, column (i), and the standard error (1.15 vs. 1.19) is similarly close. The small gap is most likely explained by a sample size difference: this replication uses 365 stores, while the paper uses 357. Those 8 extra stores are probably ones where the paper applied a stricter rule for missing wage data that is not fully documented in the public codebook.

One thing worth noting: this replication targets column (i) of Table 4, which is the bare-bones specification with only a NJ dummy. Column (ii) in the paper adds controls for chain type (Burger King, KFC, etc.) and whether the store is company-owned or franchised — and those controls barely change the NJ coefficient (2.30 vs. 2.33), which is itself a reassuring finding from the paper.
